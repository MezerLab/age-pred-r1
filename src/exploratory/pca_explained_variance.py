...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  2 11:00:40 2019

@author: Gaurav Kaushik
    https://github.com/gaurav-cascade
From original post: https://medium.com/cascade-bio-blog/creating-visualizations-to-better-understand-your-data-and-models-part-1-a51e7e5af9c0
"""
...
#        print("WARNING: number of features greater than number of instances.")
    ...
    # save and show p
    if save_plot:
        output_file(opj('/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/','PCA_analysis.html'))
    show(p)
...
#    df_abs = df_abs.sort_values(by=list(df_abs.columns), ascending=False)
    ...
    if plot_opt:
        ...
        if save_plot:
            plt.savefig(opj('/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/',
    ...
    if explained_var.any():
        ...
        if plot_opt:
            plt.figure(dpi=fig_dpi)
            ax_relative = sns.heatmap(df_byvar.iloc[:20,:10], cmap="Purples")
...
def pca_full_report(X, features_:list=None, fig_dpi:int=150, save_plot:bool=False):
    ...
    # df_n = normalized, df_na = normalized abs, df_nabv = normalized abs * variance
...
#pc2_na = df_na['PC2']
#
#pc1_nabv = df_nabv['PC1']
#pc2_nabv = df_nabv['PC2']
#
#plot_area_values(pc1_n, '/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/pc1_n_pos.jpg', cmap=plt.cm.Reds)
#plot_area_values(np.dot(pc1_n, -1), '/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/pc1_n_neg.jpg', cmap=plt.cm.Reds)
#plot_area_values(pc2_n, '/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/pc2_n_pos.jpg', cmap=plt.cm.Reds)
#plot_area_values(np.dot(pc2_n, -1), '/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/pc2_n_neg.jpg', cmap=plt.cm.Reds)
#
#plot_area_values(pc1_na, '/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/pc1_na.jpg', cmap=plt.cm.Reds)
#plot_area_values(pc2_na, '/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/pc2_na.jpg', cmap=plt.cm.Reds)
#plot_area_values(pc1_nabv, '/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/pc1_nabv.jpg', cmap=plt.cm.Reds)
#plot_area_values(pc2_nabv, '/ems/elsc-labs/mezer-a/Mezer-Lab/projects/code/wm-covar/reports/figures/exploratory/pc2_nabv.jpg', cmap=plt.cm.Reds)
#
#
#plt.scatter(X_pca[:, 1], X_pca[:, 0])
#
#plt.scatter(y_t1, data_t1[42,:])
#
#
#np.dot(df_na['PC1'], pca_evr)
...
def pca_report_interactive_fast(X, scale_X:bool=False, save_plot:bool=False):
    """
    X:          input data matrix
    scale_X:    determine whether to rescale X (StandardScaler) [default: True, X is not prescaled
    save_plot:  save plot to file (html) and not show
...
def pca_feature_correlation_fast(X, X_pca, explained_var, features:list=None, fig_dpi:int=150, save_plot:bool=False):
    """
    1. Get dot product of X and X_pca
    2. Run normalizations of X*X_pca
    3. Retrieve df/matrices
    ...
    # Normalize & Sort
    df_n, df_na, df_nabv = normalize_dataframe_fast(df_dotproduct, explained_var, plot_opt=True, save_plot=save_plot)
    
    return df_dotproduct, df_n, df_na, df_nabv


def normalize_dataframe_fast(df, explained_var=None, fig_dpi:int=150, plot_opt:bool=False, save_plot:bool=False):
    """
    1. Get z-normalized df (normalized to µ=0, σ=1)
    2. Get absolute value of z-normalized df
    3. If explained_variance matrix provided, dot it w/ (2)
...
def pca_rank_features_fast(df_nabv, verbose:bool=False):
    """
    Given a dataframe df_nabv with dimensions [f, p], where:
        f = features (sorted)
        p = principal components
...
def pca_full_report_fast(X, features_:list=None, fig_dpi:int=150, save_plot:bool=False):
    """
    Run complete PCA workflow:
        1. pca_report_interactive()
        2. pca_feature_correlation()
    ...
    """
    # Retrieve the interactive report
    df_pca, X_pca, pca_evr = pca_report_interactive_fast(X, save_plot=save_plot)
    # Get feature-PC correlation matrices
    df_corr, df_n, df_na, df_nabv = pca_feature_correlation_fast(X, X_pca, pca_evr, features_, fig_dpi, save_plot) 
    # Get rank for each feature
    df_rank = pca_rank_features_fast(df_nabv)
...
#df_pca, X_pca, pca_evr, df_corr, df_n, df_na, df_nabv, df_rank = pca_full_report_fast(X=data_t1.T, features_=areas, save_plot=False)
